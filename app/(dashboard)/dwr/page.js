...
export default function ActionCenterPage() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await actionCenterAPI.getItems({ filter: activeFilter });
  ...
  return (
    <div>
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "0 12px" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "20px", flexWrap: "wrap" }} ref={filterRef}>
          {FILTERS.map((f) => (
            <button
              key={f.key}
        ...
        {loading ? (
          ...
          <>
            {activeFilter !== "all" && activeFilter !== "pending" && activities.length > 0 && (
