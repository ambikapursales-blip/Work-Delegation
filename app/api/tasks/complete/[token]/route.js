...
async function processComplete(token) {
  ...
  if (decoded.purpose !== "complete_task") {
  ...
  if (progressEntry.status === "Completed") {
    return { success: true, message: "This task has already been completed" };
  }

  progressEntry.status = "Completed";
  progressEntry.completedAt = new Date();

  const allCompleted = task.assigneeProgress.every(
  ...
  await Notification.create({
    ...
    message: `Task "${task.title}" has been marked as completed via email`,
    type: "task_completed",
    entityId: task._id,
    entityType: "Task",
  });
  ...
  await Activity.create({
    user: userId,
    type: "task_completed",
    description: `${user.name} completed task "${task.title}" via email`,
    entityId: task._id,
    entityType: "Task",
  });
...
export async function GET(request, { params }) {
  ...
  try {
    const result = await processComplete(token);
    return htmlResponse(result.message, result.success);
  } catch (err) {
    if (err.name === "TokenExpiredError") {
      return htmlResponse("This completion link has expired", false);
    }
    if (err.name === "JsonWebTokenError") {
      return htmlResponse("Invalid completion link", false);
    }
...
function htmlResponse(message, success) {
  ...
  const title = success ? "Task Completed" : "Unable to Complete";
