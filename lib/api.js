...
export const conversationAPI = {
  ...
  getByTask: (taskId) => api.get(`/conversations/task/${taskId}`),
  getMessages: (taskId, { cursor, limit, direction } = {}) =>
    api.get(`/conversations/task/${taskId}/messages`, {
      params: { cursor, limit, direction },
    }),
  sendMessage: (taskId, data) =>
    api.post(`/conversations/task/${taskId}/messages`, data),
  editMessage: (messageId, data) =>
    api.put(`/conversations/messages/${messageId}`, data),
  deleteMessage: (messageId) =>
    api.delete(`/conversations/messages/${messageId}`),
  markAsRead: (taskId) =>
    api.put(`/conversations/task/${taskId}/read`),
...
// NOTE: createTask is DEPRECATED — returns 410 Gone. Use masterTaskAPI.createMasterTask instead.
export const taskAPI = {
  getTasks: (filters) => api.get("/tasks", { params: filters }),
  getTask: (id) => api.get(`/tasks/${id}`),
  /** @deprecated Use masterTaskAPI.createMasterTask instead — returns 410 Gone */
  ...
  requestExtension: (id, data) => api.post(`/tasks/${id}/extend`, data),
};
...
export const masterTaskAPI = {
  getMasterTasks: (filters) => api.get("/master-tasks", { params: filters }),
  getMasterTask: (id) => api.get(`/master-tasks/${id}`),
  createMasterTask: (data) => api.post("/master-tasks", data),
  updateMasterTask: (id, data) => api.put(`/master-tasks/${id}`, data),
  deleteMasterTask: (id) => api.delete(`/master-tasks/${id}`),
  pauseMasterTask: (id) => api.put(`/master-tasks/${id}/pause`),
  resumeMasterTask: (id) => api.put(`/master-tasks/${id}/resume`),
  cloneMasterTask: (id) => api.post(`/master-tasks/${id}/clone`),
  getMasterTaskHistory: (id, filters) =>
    api.get(`/master-tasks/${id}/history`, { params: filters }),
  deleteOccurrence: (id, occurrenceId) =>
    api.delete(`/master-tasks/${id}/occurrences/${occurrenceId}`),
  getStats: (dashboard) => api.get("/master-tasks/stats", { params: dashboard ? { dashboard: "true" } : {} }),
  generateNow: (id) => api.post(`/master-tasks/${id}/generate-now`),
};
